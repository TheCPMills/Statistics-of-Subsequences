...
public class App {
    ...
    private static void initializeMLC() throws Exception {
        ...
        if (n <= 0 || m <= 0) {
            ...
            // Generate model
            ModelGenerator.generate(n, m);
            objectModel = new Model("model_" + n + "x" + m + ".obj");

            // Generate model matrix
            vec3 offset = new vec3((float) -Math.pow(2, n-1), 0f, (float) Math.pow(2, m-1));
            modelMatrix = glm.rotationMatrix(new vec3(0.0f, 1.0f, 0.0f), glm.radians(90)).mult(glm.translationMatrix(offset));
        }
        ...
        if (n <= 0 || m <= 0) {
            ...
            lights.add(new PointLight(new vec3(0.0f, Math.min(n, m) + 1, 0.0f), new vec3(2.0f, 0.05f, 0.05f), 0.5f, new vec4(2.5f, 2.45f, 2.5f, 2.5f).scale(1.0f)));
        }

        // camera initialization
        perspectiveMatrix = glm.perspective(70.0f, (float) width / (float) height, 0.1f, 100.0f);
        orthoMatrix = glm.ortho(-(2.0f + n) * aspectRatio, (2.0f + n) * aspectRatio, -(2.0f + n), (2.0f + n), 0.1f, 100.0f);
        perspectiveEye = new vec3(2.0f + n, 2.0f + n, -(2.0f + n));
        orthoEye = new vec3(0.0f, 10.0f + n, 0.0f);
