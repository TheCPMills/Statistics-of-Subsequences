    ...
    private static void genObj(int n, int m, FileWriter objWriter) throws Exception {
        ...
        for(int x = 1; x < lcsMatrix.length - 1; x++) {
            for (int z = 1; z < lcsMatrix[x].length - 1; z++) {
                ...
                // create object mesh
                CubeObjectGroup object = new CubeObjectGroup(id, front, back, right, left, top, bottom, material);
                objects.add(object);
            }
        }

        // create labels
        float nAllZeroesWidth = 0.108f * n + 0.0125f * (n - 1);
        float mAllZeroesWidth = 0.108f * m + 0.0125f * (m - 1);
        float limit = 0.60f;
        float scaleFactor = (float) Math.min(1.25f, limit / Math.max(nAllZeroesWidth, mAllZeroesWidth));

        // center each label to each cell
        vec3 position = vec3.ZERO;
        for (int x = 0; x < Math.pow(2, n); x++) {
            String label = Integer.toBinaryString(x);
            ...
            for (int bit = 0; bit < n; bit++) {
                int currentBit = label.charAt(bit) - '0';
                if (currentBit == 0) {
                    labelWidth += (bit + 1 == n) ? 0.108f : 0.108f + 0.0125f;
                } else {
                    labelWidth += (bit + 1 == n) ? 0.052f : 0.052f + 0.025f;
                }
            }

            position = new vec3((1.0f - labelWidth * scaleFactor) / 2.0f + x, 0.0f, -0.5f);

            ObjectGroup object = new ObjectGroup("Label(x," + label + ")[string_label]", new ArrayList<Face>(), "string_label");
            for (int bit = 0; bit < n; bit++) {
                int currentBit = label.charAt(bit) - '0';

                Quartet<List<Face>, vec3, List<vec3>, Integer> result;
                if (currentBit == 0) {
                    result = BinaryStringModelGenerator.modelZero(bit, position, scaleFactor, vertices, nextVertexIndex);
                } else {
                    result = BinaryStringModelGenerator.modelOne(bit, position, scaleFactor, vertices, nextVertexIndex);
                }

                List<Face> faces = result.getValue0();
                position = result.getValue1();
                vertices = result.getValue2();
                nextVertexIndex = result.getValue3();
        ...
        for (int y = 0; y < Math.pow(2, m); y++) {
            String label = Integer.toBinaryString(y);
            while (label.length() < m) {
                label = "0" + label;
            }
            ...
            for (int bit = 0; bit < m; bit++) {
                int currentBit = label.charAt(bit) - '0';
                if (currentBit == 0) {
                    labelWidth += (bit + 1 == m) ? 0.108f : 0.108f + 0.0125f;
                } else {
                    labelWidth += (bit + 1 == m) ? 0.052f : 0.052f + 0.025f;
                }
            }

            position = new vec3(-0.25f - labelWidth * scaleFactor, 0.0f, (1.0f - 0.185f * scaleFactor) / 2.0f + y);

            ObjectGroup object = new ObjectGroup("Label(y," + label + ")[string_label]", new ArrayList<Face>(), "string_label");
            for (int bit = 0; bit < m; bit++) {
                int currentBit = label.charAt(bit) - '0';
            ...
            objects.add(object);
            position = new vec3((float) Math.pow(2, n) + 1, 0.0f, y + 1);
        }
