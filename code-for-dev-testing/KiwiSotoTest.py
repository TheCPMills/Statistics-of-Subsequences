...
def F(v1, v2, length):
    ...
    # b + max
    b_combined = np.concatenate((1 + 0.25 * f_double, 0.5 * f01))
    return b_combined
...
def F_01(v, length):
    ...
    for st in range(start, 2 * start):
        ...
        print(st, ":  ", ATB0, ATB1, TA0B, TA1B)

        ret[st - start] = max(ret[st-start], v[TA0B] + v[TA1B])  # if h(A) != h(B) and h(B) = z
