...
#define CALC_EVERY_X_ITERATIONS 5
...
void F_01_combined(const uint64_t start, const uint64_t end, const ArrayXd &v, ArrayXd &ret, const double R,
    ...
    for (uint64_t str = start; str < end; str++) {
        ...
        const uint64_t str3 = powminus0 - 1 - str;
        ...
        cout << str << ": " << ATB0 << " " << ATB1 << " " << TA0B3 << " " << TA1B3 << endl;
        cout << v[ATB0] << " " << v[ATB1] << " " << v[TA0B3] << " " << v[TA1B3] << endl;
...
void FeasibleTriplet(int n) {
    ...
    for (int i = 2; i < n + 1; i++) {
        ...
        F(v1, v1, v2);
        printArray(v2);
        double e = subtract_and_find_min_parallel(v1, v2);
        cout << "e result: " << (2.0 * e / (1 + e)) << endl;
