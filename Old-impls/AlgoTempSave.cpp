...
void F(const ArrayXd &v1, const ArrayXd &v2, ArrayXd &ret) {
    ...
    ArrayXd f01;
    F_01(v1, f01);  // potential optimization? perform .5 multiply locally?
    ArrayXd f11 = f01.reverse();
    ArrayXd f_double;
    F_12(v2, f_double);
    ...
    ret = b + ((0.5 * f01) + (0.25 * f_double)).max((0.5 * f11) + (0.25 * f_double.reverse()));
}
