...
# note: v has size n (e.g. i in {0,...n})
# 0_v corresponds to a vector of size 2^2l representing every possible string value for both strings
def FeasibleTriplet(length, n):
    ...
    for i in range(2, n + 1):
        ...
        R = max(v_i - v[i - 1])
        W = v_i + 2 * sum(R) - F(v_i + sum(R), v_i, length)
        E = max(0, max(W))
...
def F(v1, v2, length):
    return np.concatenate((np.ones(2**(2*length - 2)), np.zeros(2**(2*length - 1)), np.ones(2**(2*length - 2)))) \
           + max(F_z(0, v1, v2, length), F_z(1, v1, v2, length))
...
def F_z1(z, v, length):
    ...
    #    & 0x2AAAAAAA = even bits except for first, T(A)
    # st & 0x55555555 = only odd bits of st, corresponding to bits in B
    #    & 0x15555555 = odd bits except for first, T(B)
    for st in range1:
        # For st = (A, B),
        # v[(A, T(B)0)] + v[(A, T(B)1)]
        ATB0 = (st & 0xAAAAAAAA) | ((st & 0x15555555) << 2)
    ...
    for st in range2:
        # For st = (A, B),
        # v[(T(A)0, B)] + v[(T(A)1, B)]
        TA0B = (st & 0x55555555) | ((st & 0x2AAAAAAA) << 2)
...
def F_z2(z, v, length):
    ...
    for st in rangeF:
        ...
        TA0TB0 = ((st & 0x15555555) << 2) | ((st & 0x2AAAAAAA) << 2)
        TA0TB1 = TA0TB0 | 0b1
        TA1TB0 = TA0TB0 | 0b10
        TA1TB1 = TA0TB0 | 0b11
